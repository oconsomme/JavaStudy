...
public class Person {
	...
	// 설계도를 만들기 위해서 클래스를 만들면, 
	// 그 안에 자동으로 생성자가 만들어진다. --> 생략
	// 기본 생성자 : 생략가능하다.
//	public Person() {
//		// 기본 생성자의 모양 (메서드랑 비슷하게 생김..? > 생성자도 메서드다...)
//	}
	...
	// * 생성자의 특징
	// 1) 생성자도 메서드
	// 2) 생성자의 이름(메서드 명) 클래스의 이름과 같아야한다.
	// : 대소문자 구분 필요!@!
	// 3) 클래스를 처음 만들 때 기본 생성자는 생략되어있다.
	// 4) 새로운 생성자를 만들면 생략되어있는 기본 생성자는 사용할 수 없다.
	// 기본 생성자도 사용하고 싶으면 눈에 보이게 코드로 작성해야한다.
	public Person() {}
	public Person(String name, int age, String gender, int height, String local) {
		// main에서 보내온 전달 인자를 매개 변수로 받아서 
